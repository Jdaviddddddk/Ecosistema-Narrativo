...
// Verifica JWT de Google y adjunta req.user
...
// Verifica JWT y además comprueba que el email sea admin
...
app.post('/api/projects', requireAuth, upload.fields([{ name: 'images', maxCount: 20 }, { name: 'pdf', maxCount: 1 }]), async (req, res) => {
  try {
    const projectData = JSON.parse(req.body.data || '{}');
    // Forzar authorId y authorEmail desde el token verificado
    projectData.authorId = req.user.sub;
    projectData.authorEmail = req.user.email;
...
app.put('/api/projects/:id', requireAuth, (req, res) => {
  ...
  const isOwner = projects[idx].authorId === req.user.sub;
  const isAdminUser = ADMIN_EMAILS.includes(req.user.email);
  if (!isOwner && !isAdminUser) return res.status(403).json({ error: 'No autorizado' });
...
app.delete('/api/projects/:id', requireAuth, (req, res) => {
  ...
  if (!project) return res.status(404).json({ error: 'No encontrado' });
  const isOwner = project.authorId === req.user.sub;
  const isAdminUser = ADMIN_EMAILS.includes(req.user.email);
  if (!isOwner && !isAdminUser) return res.status(403).json({ error: 'No autorizado' });
  writeJSON(dbFile, projects.filter(p => p.id !== req.params.id));
  res.json({ ok: true });
});
...
app.post('/api/projects/:id/comments', requireAuth, (req, res) => {
  ...
  // authorId siempre viene del token, no del cliente
  const comment = { id: Date.now().toString(), projectId: req.params.id, authorId: req.user.sub, authorName, authorAvatar, text: text.trim(), ...(parentId && { parentId }), createdAt: new Date().toISOString() };
  const all = readJSON(commentsFile);
  all.push(comment);
...
app.delete('/api/projects/:id/comments/:commentId', requireAuth, (req, res) => {
  const all = readJSON(commentsFile);
  const comment = all.find(c => c.id === req.params.commentId && c.projectId === req.params.id);
  if (!comment) return res.status(404).json({ error: 'No encontrado' });
  const isOwner = comment.authorId === req.user.sub;
  const isAdminUser = ADMIN_EMAILS.includes(req.user.email);
  if (!isOwner && !isAdminUser) return res.status(403).json({ error: 'No autorizado' });
  writeJSON(commentsFile, all.filter(c => !(c.id === req.params.commentId && c.projectId === req.params.id)));
  res.json({ ok: true });
});
...
app.post('/api/users', requireAuth, (req, res) => {
  ...
  if (req.body.id && req.body.id !== req.user.sub) {
    return res.status(403).json({ error: 'No autorizado' });
  }
  const users = readJSON(usersFile);
  const idx = users.findIndex(u => u.id === req.user.sub);
  const userData = { ...req.body, id: req.user.sub, updatedAt: new Date().toISOString() };
...
// ── CURADURÍA IA ──
app.post('/api/curate', isAdmin, async (req, res) => {
  try {
